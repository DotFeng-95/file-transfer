%  Fig_gen.m
%
%  Generate 2-dimensional TV image reconstruction in chapter 5.
%
...
%  the gradient.
  ...
  if ~exist('max_fp_iter')
    alpha = 10; %%%input(' Regularization parameter alpha = ');
    beta = 1; %%%input(' TV smoothing parameter beta = ');
    max_fp_iter = 10; %%%input(' Max. no. of fixed point iterations = ');
    max_cg_iter = 500; %%%input(' Max. no. of CG iterations = ');
    cg_steptol = 1e-4;
    cg_residtol = 1e-4;
  ...
  for fp_iter = 1:max_fp_iter
    ...
    u_fp = U_fp(:);
    psi_prime1 = psi_prime((Dx1*u_fp).^2 + (Dy1*u_fp).^2, beta);
    psi_prime2 = psi_prime((Dx2*u_fp).^2 + (Dy2*u_fp).^2, beta);
