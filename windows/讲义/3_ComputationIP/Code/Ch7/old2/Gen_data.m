...
%  Fourier transform.
  ...
  PSF = abs(ifft2(H)).^2;
  OTF = dft(PSF);            %  Optical transfer function.
  OTF = OTF / OTF(1,1);
  PSF = real(idft(OTF));
keyboard
...
%      data = convolution(PSF,f_true) + noise. 
  ...
  dat_true = convolve_2d(OTF, f_true);  %  Convolve PSF with f_true.
  error_percnt = input(' Percent data error = ');
  stdev = .01 * error_percnt * norm(dat(:)) / sqrt(nx*ny);
  eta = stdev*randn(nx,ny);     %  Generate Gaussian noise.
  dat = dat_true + eta;         %  Add noise to data.
...
%  Display data.
  ...
  figure(3)
      imagesc(fftshift(log(abs(OTF) + sqrt(eps) ))), colorbar
