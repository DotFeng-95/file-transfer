...
%  constraint max_i sqrt(Vx(i)^2 + Vy(i)^2) < 1 is maintained.
 ...
 if reset_flag == 1
  ...
  for pd_iter = 1:max_pd_iter
    ...
    uu = u(:); vv = v(:);
    w = 2 * psi_2 ./ psi_1.^2;
    E11 = spdiags(1 + w.*uu.*Dxf, 0, (n-1)^2,(n-1)^2);
    E12 = spdiags(w.*uu.*Dyf, 0, (n-1)^2,(n-1)^2);
    E21 = spdiags(w.*vv.*Dxf, 0, (n-1)^2,(n-1)^2);
    E22 = spdiags(1+ w.*vv.*Dyf, 0, (n-1)^2,(n-1)^2);
    ...
    [Delf,residnormvec,stepnormvec,cgiter] = ...
      cg(k_hat_sq,Lbar,alph_Dxy,r,max_cg_iter,cg_steptol,cg_residtol);
    ...
    DxDf = Dx*Delf;
    DyDf = Dy*Delf;
    Delta_u = -u(:) + Binv*(Dx*f_pd(:) + E11*DxDf + E12*DyDf);
    Delta_u = reshape(Delta_u,n-1,n-1);
    Delta_v = -v(:) + Binv*(Dy*f_pd(:) + E21*DxDf + E22*DyDf);
